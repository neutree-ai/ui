...
const RankingPanel = ({
  ...
  const top = entries
    ...
    .slice(0, 3);
  const others = total - top.reduce((sum, entry) => sum + entry.value, 0);
  const pie = [
    ...top,
...
export const ApiKeyRankingOverview = ({
  ...
  const { t } = useTranslation();
  const requestEntries = keys.map((key) => ({
    name: key.name,
    value: traffic.get(key.id)?.requests ?? 0,
  }));
  const tokenEntries = keys.map((key) => ({
    name: key.name,
    value: traffic.get(key.id)?.tokens ?? 0,
  }));
