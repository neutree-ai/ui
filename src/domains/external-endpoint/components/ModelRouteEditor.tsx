...
function modeOf(route: ModelRoute): Mode {
  ...
  if (route.targets.some((target) => (target.priority ?? 0) !== 0)) return "priority";
  return "weighted";
}
...
export default function ModelRouteEditor({ value = [], onChange, upstreams }: Props) {
  const { t } = useTranslation();
  const providers = upstreams.map((upstream, index) => ({
    label: upstream.name || `${t("external_endpoints.fields.provider")} ${index + 1}`,
  ...
  return (
    <div className="space-y-4">
      {value.map((route, index) => {
        const mode = modeOf(route);
        return (
          <div key={`${route.model}-${index}`} className="rounded border border-border/60 p-4 space-y-4">
            <div className="grid grid-cols-2 gap-4 xs:grid-cols-1">
              <Input
                value={route.model}
                onChange={(event) => update(index, { ...route, model: event.target.value })}
              ...
              <FormSelect
                ...
                options={[
                  { label: t("external_endpoints.options.fixedRouting"), value: "fixed" },
                  { label: t("external_endpoints.options.priorityRouting"), value: "priority" },
                  { label: t("external_endpoints.options.weightedRouting"), value: "weighted" },
                ]}
              />
            </div>
            <div className="space-y-2">
              {route.targets.map((_, targetIndex) => {
                ...
                return (
                  <div key={targetIndex} className="grid grid-cols-[1fr_1fr_auto] gap-3 xs:grid-cols-1">
                    <FormSelect
                      ...
                      options={providers}
                      placeholder={t("external_endpoints.placeholders.selectProvider")}
                    />
                    <Input
                      value={target.upstream_model}
                      onChange={(event) => {
                        const targets = route.targets.slice();
                        targets[targetIndex] = { ...target, upstream_model: event.target.value };
                        update(index, { ...route, targets });
                      }}
                    ...
                    <div className="flex items-center gap-2">
                      {mode === "weighted" && (
                        <Input
                          ...
                          onChange={(event) => {
                            const targets = route.targets.slice();
                            targets[targetIndex] = { ...target, weight: Number(event.target.value) || 1 };
                            update(index, { ...route, targets });
                          }}
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        onClick={() => update(index, { ...route, targets: route.targets.filter((_, i) => i !== targetIndex) })}
              ...
              {mode !== "fixed" && (
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => update(index, { ...route, targets: [...route.targets, { upstream: "", upstream_model: "" }] })}
                >
            ...
            <div className="grid grid-cols-2 gap-4 xs:grid-cols-1">
              <FormSelect
                ...
                onChange={(next) => update(index, next === "enabled"
                  ? { ...route, retryable_conditions: ["http_429", "http_502", "http_503", "http_504", "timeout"], max_attempts: 1 }
                  : { ...route, retryable_conditions: [], max_attempts: 0 })}
                options={[
                  { label: t("external_endpoints.options.noRetry"), value: "disabled" },
                  { label: t("external_endpoints.options.retryProvider"), value: "enabled" },
                ]}
              />
            </div>
          </div>
      ...
      <Button
        type="button"
        variant="outline"
        onClick={() => onChange?.([...value, { model: "", targets: [{ upstream: providers[0]?.value || "", upstream_model: "" }] }])}
      >
        <Plus className="mr-1 h-4 w-4" />
