...
export const useClusterForm = ({ action }: { action: "create" | "edit" }) => {
  ...
  const validateK8sName = (value: string) => {
    if (!value) return true; // Allow empty for optional fields
    const k8sNameRegex = /^[a-z0-9]([-a-z0-9]*[a-z0-9])?$/;
    if (k8sNameRegex.test(value) && value.length <= 253) {
      return true;
    }
    return (
      t("clusters.validation.invalidK8sName") ||
      "Invalid name format (lowercase alphanumeric and hyphens, max 253 characters)"
    );
  };
