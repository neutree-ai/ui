...
test.describe("image registries", () => {
  ...
  test.describe("create", () => {
    ...
    test("URL without protocol → server rejects or status becomes Failed", {
      tag: "@C2612016",
    }, async ({ imageRegistries, apiHelper }) => {
...
spec:
  ...
  test.describe("connection and status", () => {
    ...
    test("Docker Hub public URL, no auth → status becomes Connected", {
      tag: "@C2612047",
    }, async ({ imageRegistries }) => {
    ...
    test("wrong auth credentials → status becomes Failed", {
      tag: "@C2612051",
    }, async ({ imageRegistries, apiHelper }) => {
