...
test.describe("endpoints", () => {
  ...
  test.describe("create", () => {
    ...
    test("create form shows template fields", { tag: "@C2613239" }, async ({
      ...
      await expect(endpoints.form.field("spec.engine.version")).toBeVisible();
      await expect(endpoints.form.field("spec.model.task")).toBeVisible();
    });

    test("create form defaults to standard deployment mode", async ({
      endpoints,
    }) => {
      await endpoints.goToCreate();

      await expect(
        endpoints.page.getByRole("button", { name: /^standard$/i }),
    ...
    test("prefill/decode mode shows role settings without KV settings", async ({
      ...
      await endpoints.page
        .getByRole("button", { name: /prefill \/ decode/i })
        .click();
