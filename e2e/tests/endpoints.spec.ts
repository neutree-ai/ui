...
test.describe("endpoints", () => {
  test.beforeAll(async ({ browser }) => {
    ...
    await api.createEngine(engineName.value, {
      valuesSchema: {
        properties: {
          speculative_config: {
            type: "object",
  ...
  // ────────────────────────────────────────────────────────────
  test.describe("list", () => {
    test("list page shows expected columns", { tag: "@C2613236" }, async ({
      endpoints,
    }) => {
    ...
    test("can sort by updated at", { tag: "@C2613257" }, async ({
      endpoints,
    }) => {
      await endpoints.goToList();
    ...
    test("can sort by created at", { tag: "@C2613258" }, async ({
      endpoints,
    }) => {
      await endpoints.goToList();
    ...
    test("cluster column shows cluster name", { tag: "@C2613256" }, async ({
      endpoints,
    }) => {
      await endpoints.goToList();
    ...
    test("engine column navigates to engine detail", {
      tag: "@C2613255",
    }, async ({ endpoints }) => {
      await endpoints.goToList();
    ...
    test("workspace column navigates to workspace detail", {
      tag: "@C2613251",
    }, async ({ endpoints }) => {
      await endpoints.goToList();
    ...
    test("list shows endpoints in current workspace", {
      tag: "@C2613302",
    }, async ({ endpoints }) => {
      await endpoints.goToList();
    ...
    test.skip("actions include Pause", { tag: "@miss" }, async ({
      endpoints,
    }) => {
      await endpoints.goToList();
  ...
  test.describe("detail", () => {
    test("show page displays basic info", { tag: "@C2612911" }, async ({
      endpoints,
    }) => {
      ...
      // Task
      await expect(showPage.locator("dt", { hasText: /task/i })).toBeVisible();
    ...
    test("clicking cluster navigates to cluster detail", {
      tag: "@C2613256",
    }, async ({ endpoints }) => {
      await endpoints.goToShow(epNames.base);
  ...
  test.describe("create", () => {
    test("create form shows basic fields", { tag: "@C2613237" }, async ({
      endpoints,
    }) => {
    ...
    test("create form shows template fields", { tag: "@C2613239" }, async ({
      endpoints,
    }) => {
      await endpoints.goToCreate();
    ...
    test("create form shows resource fields", { tag: "@C2613240" }, async ({
      ...
      await expect(endpoints.form.field("spec.resources.memory")).toBeVisible();
      await expect(
    ...
    test("CPU and memory disabled until cluster selected", {
      tag: "@C2613264",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("model name disabled until registry selected", {
      tag: "@C2613269",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("model catalog field visible in create mode", {
      tag: "@C2613266",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("model version: default empty, can input", {
      tag: "@C2613271",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("model file field visible", { tag: "@C2613272" }, async ({
      endpoints,
    }) => {
      await endpoints.goToCreate();
    ...
    test("replicas default value is 1", { tag: "@C2613300" }, async ({
      endpoints,
    }) => {
      await endpoints.goToCreate();
    ...
    test("scheduler type field visible", { tag: "@C2613301" }, async ({
      endpoints,
    }) => {
      await endpoints.goToCreate();
    ...
    test("replicas = 0 shows validation error", { tag: "@C2613307" }, async ({
      endpoints,
    }) => {
      await endpoints.goToCreate();
    ...
    test("replicas negative shows validation error", {
      tag: "@C2613308",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("advanced options: engine variables section visible", {
      tag: "@C2613242",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("engine selection list includes vllm", { tag: "@C2613273" }, async ({
      endpoints,
    }) => {
      await endpoints.goToCreate();
    ...
    test("add engine variables", { tag: "@C2613293" }, async ({
      endpoints,
    }) => {
      await endpoints.goToCreate();
      ...
      const valueInput = engineVarsField.getByPlaceholder(/new value/i).first();
    ...
    test("create: catalog selection populates fields when customize section already open", {
      tag: "@C2626011",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("create: switching catalog replaces previous template values", {
      tag: "@C2626012",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("create: workspace defaults to 'default'", {
      tag: "@C2613238",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("create: workspace auto-selects current user workspace", {
      tag: "@C2613303",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("create: cancel navigates back to list", { tag: "@C2613244" }, async ({
      endpoints,
    }) => {
      await endpoints.goToCreate();
      ...
      await expect(endpoints.page.locator('[data-testid="form"]')).toBeHidden();
    });

    test("create: CPU and memory enabled after cluster selected", {
      tag: "@C2613265",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("create: engine selection auto-loads version and task", {
      tag: "@C2613274",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("create: object engine_args are submitted as structured JSON", {
      tag: "@C2724227",
    }, async ({ endpoints, apiHelper }) => {
      await endpoints.goToCreate();

      const name = `test-ep-json-${Date.now()}`;
      ...
      try {
        ...
        const engineField = endpoints.form.field("spec.engine.engine");
        if (!(await engineField.isVisible())) {
          await endpoints.page
            .getByRole("button", { name: /customize settings/i })
            .click();
        }
        ...
        await firstRow.getByRole("textbox").first().fill("speculative_config");

        const jsonInput = firstRow.locator("textarea");
        await expect(jsonInput).toBeVisible();
        await jsonInput.fill(
          '{"method":"mtp","num_speculative_tokens":2,"nested":{"enabled":true}}',
        );
        await jsonInput.blur();
        await endpoints.page.keyboard.press("Escape");

        const savedObjectRow = engineVarsField
          .locator("tbody tr")
          .filter({ hasText: "speculative_config" });
        await expect(savedObjectRow).toBeVisible();
        await expect(savedObjectRow.locator("textarea")).toHaveValue(
          /"method": "mtp"/,
        );

        const requestPromise = endpoints.page.waitForRequest(
          (request) =>
            request.url().includes("/endpoints") && request.method() === "POST",
        );
        ...
        shouldCleanup = response.ok();
        const body = JSON.parse(request.postData() || "{}");

        expect(
          body.spec.variables.engine_args.speculative_config,
        ).toMatchObject({
          method: "mtp",
          num_speculative_tokens: 2,
          nested: { enabled: true },
        });
        expect(typeof body.spec.variables.engine_args.speculative_config).toBe(
          "object",
        );
      } finally {
        if (shouldCleanup) {
    ...
    test("create: HF model registry loads model list", {
      tag: "@C2613296",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("create: filesystem model registry loads model list", {
      tag: "@C2613297",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
    ...
    test("create: save submits all parameters", { tag: "@C2613243" }, async ({
      endpoints,
      apiHelper,
    }) => {
      await endpoints.goToCreate();
    ...
    test("create: empty required fields → submit shows error", {
      tag: "@C2613306",
    }, async ({ endpoints }) => {
      await endpoints.goToCreate();
  ...
  // ────────────────────────────────────────────────────────────
  test.describe("edit", () => {
    test("edit: name and workspace disabled, collapsible title changes", {
      tag: "@C2613286",
    }, async ({ endpoints }) => {
      ...
      // Name disabled
      const nameInput = endpoints.form.field("metadata.name").locator("input");
    ...
    test("edit: model catalog field hidden", { tag: "@C2613290" }, async ({
      endpoints,
    }) => {
      await endpoints.goToEdit(epNames.base);
      await expect(
    ...
    test("edit: resource config fields editable", { tag: "@C2613289" }, async ({
      endpoints,
    }) => {
      await endpoints.goToEdit(epNames.base);
      await expect(
    ...
    test("edit: change engine updates version and task", {
      tag: "@C2613291",
    }, async ({ endpoints }) => {
      await endpoints.goToEdit(epNames.base);
      await expect(
    ...
    test("edit: replicas and scheduler type editable", {
      tag: "@C2613292",
    }, async ({ endpoints }) => {
      await endpoints.goToEdit(epNames.base);
      await expect(
    ...
    test("edit: delete engine variable", { tag: "@C2613294" }, async ({
      endpoints,
    }) => {
      await endpoints.goToEdit(epNames.base);
      await expect(
  ...
  test.describe("create permissions", () => {
    test("admin can create endpoint", { tag: "@C2613363" }, async ({
      endpoints,
      apiHelper,
    }) => {
  ...
  test.describe("edit permissions", () => {
    test("admin can edit endpoint", { tag: "@C2613367" }, async ({
      endpoints,
      apiHelper,
    }) => {
  ...
  test.describe("list permissions", () => {
    test("admin can view endpoint list", { tag: "@C2613378" }, async ({
      endpoints,
    }) => {
      await endpoints.goToList();
