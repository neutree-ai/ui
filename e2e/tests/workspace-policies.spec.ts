...
test.describe("workspace policies detail", () => {
  test("detail page shows policy info with workspace, role, and user", {
    ...
    await expect(showPage.locator("dt", { hasText: /^user$/i })).toBeVisible();
  });
});

test.describe("workspace policies create", () => {
  test("admin can create policy with all fields", {
  ...
  test("user dropdown shows existing users", {
    tag: "@C2611747",
  }, async ({ workspacePolicies: wp }) => {
    await wp.goToCreate();
  ...
  test("user combobox supports search", {
    tag: "@C2611763",
  }, async ({ workspacePolicies: wp }) => {
    await wp.goToCreate();
  ...
  test("role dropdown shows existing roles", {
    tag: "@C2611748",
  }, async ({ workspacePolicies: wp }) => {
    await wp.goToCreate();
  ...
  test("role combobox supports search", {
    tag: "@C2611764",
  }, async ({ workspacePolicies: wp }) => {
    await wp.goToCreate();
  ...
  test("policy scope defaults to Global and is disabled in open source", {
    ...
    await expect(scopeField.locator('button[role="combobox"]')).toBeDisabled();
  });
...
test.describe("workspace policies edit", () => {
  test("admin can edit and save policy changes", {
    ...
    await wp.table.editRow(name);
    await expect(wp.page.locator('[data-testid="form-submit"]')).toBeEnabled();
  ...
  test("policy scope is disabled in open source edit", {
...
test.describe("workspace policies delete", () => {
  ...
  test("can delete from detail page", {
    tag: "@C2611788",
  }, async ({ workspacePolicies: wp }) => {
  ...
  test("admin-global-role-assignment has no actions", {
    tag: "@C2611789",
  }, async ({ workspacePolicies: wp }) => {
    await wp.goToList();
